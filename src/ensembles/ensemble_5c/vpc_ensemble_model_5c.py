import tensorflow
from tensorflow.keras.layers import LeakyReLU, BatchNormalization, Dropout, Dense, Input
from tensorflow.keras.layers import Concatenate
from tensorflow.keras.models import  Model
from tensorflow.keras.optimizers import Adam
from tensorflow.keras.callbacks import ModelCheckpoint, EarlyStopping,  ReduceLROnPlateau
from tensorflow.keras.utils import plot_model
from vpc_cnn_c import CNN_C
import vpc_data
...
class EnsembleModel5C:
    ...
    def __init__(self, model_name: str, img_size: int) -> None:
        ...
        for i in range(5):
            ...
            modeli =  CNN_C(f"MemberModelC{i}", img_size=30, channels=1)
    ...
    def train_ensemble(self, epochs, stop_early=True):
        ...
        X_train_grayscale, y_train, X_val_grayscale, y_val =  vpc_data.load_training_data(validation_split=0.2)
        ...
        gen_flow = generate_data_generator(X_train_grayscale, y_train)

        self.training_history = self.stacked_model.fit_generator(
            gen_flow,
            epochs=epochs,
            validation_data=([X_val_grayscale, X_val_grayscale, X_val_grayscale, X_val_grayscale, X_val_grayscale], y_val),
            callbacks=callbacks,
            steps_per_epoch=len(X_train_grayscale) / BATCH_SIZE,
            verbose=1
        )
    ...
    def evaluate_ensemble(self, X_test_grayscale, y_test):
        

        pred = self.stacked_model.predict([X_test_grayscale, X_test_grayscale, X_test_grayscale, X_test_grayscale, X_test_grayscale])
...
def generate_data_generator(gray_scale_images, y):
    genX1 = gen.flow(gray_scale_images, y,  batch_size=BATCH_SIZE,seed=42)
    genX2 = gen.flow(gray_scale_images,gray_scale_images, batch_size=BATCH_SIZE,seed=42)
    genX3 = gen.flow(gray_scale_images,gray_scale_images, batch_size=BATCH_SIZE,seed=42)
    genX4 = gen.flow(gray_scale_images,gray_scale_images, batch_size=BATCH_SIZE,seed=42)
    genX5 = gen.flow(gray_scale_images,gray_scale_images, batch_size=BATCH_SIZE,seed=42)
    while True:
            X1i = genX1.next()
            X2i = genX2.next()
            X3i = genX3.next()
